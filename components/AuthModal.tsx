...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, initialMode, onClose }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setLoading(false);
      alert(`${mode === 'login' ? 'Logged In' : 'Signed Up'} Successfully! (This is a demo)`);
      onClose();
    }, 1500);
  };
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in-up">
        ...
        <form onSubmit={handleSubmit} className="px-8 pb-8 space-y-4">
          ...
          <button 
            type="submit"
            disabled={loading}
            className="w-full bg-brand-gradient text-white py-3.5 rounded-xl font-bold text-sm shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 flex items-center justify-center gap-2 mt-6"
          >
            {loading ? (
